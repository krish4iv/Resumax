...
import { X, Plus, Trash2, Sparkles, Loader2, Calendar, Check } from "lucide-react"
...
export default function ExperienceModal({ open, initial, onClose, onSave, onDelete }) {
  const [form, setForm] = useState(emptyExperience)
  const [rewritingIndex, setRewritingIndex] = useState(null)
  const [pendingRewrites, setPendingRewrites] = useState({}) 

  useEffect(() => {
    if (open) setForm(initial ? { ...emptyExperience, ...initial } : emptyExperience)
  }, [open, initial])
...
function dismissRewrite(i) {
  ...
  function handleSave() {
    ...
    onSave({ ...form, bullets: form.bullets.filter(b => b.trim()) })
  }
        ...
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input className={inputStyle} value={form.role} onChange={e => update("role", e.target.value)} placeholder="Frontend Developer" />
            </div>
            <div>
              <label className={labelStyle}>Company *</label>
              <input className={inputStyle} value={form.company} onChange={e => update("company", e.target.value)} placeholder="Acme Inc" />
            </div>
          </div>

          <div>
            <label className={labelStyle}>Location</label>
            <input className={inputStyle} value={form.location} onChange={e => update("location", e.target.value)} placeholder="Remote / San Francisco, CA" />
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className={labelStyle}>Achievements & Responsibilities</label>
              <button
                onClick={addBullet}
